...
| `worker/src/dna_entropy/config.py` | `RunConfig` dataclass, `PredictorKind`/`TrackFormat` enums, `DEFAULT_MAX_LEN=8192`. | ADAPT | Needs new fields for context length `K`, direction, window/stride (derived), and a progress-callback hook per §5.5/§5.6; CLI-flag-shaped fields (`out_dir`, `informat`) become manifest fields instead. | Stays in `worker/src/dna_entropy/config.py`; extended, not replaced. **DONE (#279):** added `Direction` enum (5 modes), `context_length` (K), `direction`, and `max_total_len` (decoupled from `max_len`, which now means the per-window GPU ceiling, not a whole-input cap). Progress-callback hook and manifest-field migration are still open (belong to #278). |
| `worker/src/dna_entropy/pipeline.py` | Orchestrates input -> validate -> predict -> analyze -> export; the only module that knows stage order; builds output sets for GenBank vs FASTA/paste. | ADAPT | Needs a progress callback (stage list from §4.4: checking, analysing per contig/window, saving), and must call the new windowing/direction logic per contig instead of one bare `predictor.predict()` call. | Stays; extended by the new `worker/run` module that wraps it with manifest I/O. **DONE (#279):** `run()` now calls `analysis.windowing.validate_context` (pushback) and `analysis.direction.analyze_direction` per contig instead of one bare `predictor.predict()`; `RunResult`/`SummaryWriter` record the derived window/stride/seam/direction provenance. Progress-callback hook is still open (belongs to #278). **DONE (#283):** `_write_standard_outputs` now takes the whole `processed` list (one `(Contig, DirectionResult)` per record) and calls every writer's `write_multi`, matching `_write_genbank_outputs`'s shape — a multi-record FASTA gets one block per contig in every output file, not just the first. |
| `worker/src/dna_entropy/cli.py` | Typer CLI: `version`, `run`, `validate`, `cloudrun`, `keep-gpu`; `_try_local_evo` for `--prefer-local`. | ADAPT | `cloudrun`/`keep-gpu`/`--prefer-local` and their gcloud/SSH plumbing are gone under the new design (direct Google APIs from C#, no gcloud); `run`/`validate` stay as the basis for the worker's manifest-driven entrypoint. | Drop `cloudrun`, `keep-gpu`, `_try_local_evo`; keep `run`/`validate` shape as the seed for the new worker CLI entrypoint. **DONE (#277):** `cloudrun`/`keep-gpu`/`_try_local_evo` removed; a `worker-run --manifest` stub command added (exits non-zero, points at #278) as the seed of the manifest-driven entrypoint. |
...
| `worker/src/dna_entropy/cloud/__init__.py` | Re-exports gcloud/orchestrator/ui symbols. | GUT | The whole `cloud/` package shells out to the user's `gcloud`/SSH, which the approved design forbids (§3: "Direct Google APIs from C#. No gcloud, no SSH."); the C# `DnaEntropyGraph.Cloud` project replaces it entirely. | Not shipped in the worker container; retained untouched for reference (see below). **DONE (#280):** moved verbatim to `worker/legacy/cloud/__init__.py`. |
| `worker/src/dna_entropy/cloud/gcloud.py` | Thin `gcloud` CLI wrappers: account/project, VM create/find/start/stop/delete, SSH/SCP, error classification, billing/API/quota checks. | GUT | Superseded by C# Compute/Storage/Billing/Quotas API gateways under `DnaEntropyGraph.Cloud` (design §3.1, §5.2). | GUT from the shipped package; **REFERENCE** for its error-classification buckets, quota metric names, and health-check logic — see "Prototype behaviours" below. **DONE (#280, #284):** moved verbatim to `worker/legacy/cloud/gcloud.py`; `classify_create_error`/`list_region_gpu_quota`/`gpu_quota_metric` cases captured in `worker/tests/contract-fixtures/cloud_error_classification.json` and `cloud_quota_parsing.json`. |
| `worker/src/dna_entropy/cloud/keeper.py` | Always-on GPU keeper: acquire/start/watchdog loop, GCP setup health report, region quota pre-filter, GPU health probe (`nvidia-smi`). | GUT | The always-on singleton VM (`dna-entropy-box`) is explicitly removed by the design (D1: "Per-job VM, found by label, never by fixed name... no shared singleton"). | GUT from the shipped package; **REFERENCE** for the setup health check, quota pre-filter, and watchdog backoff logic — see "Prototype behaviours" below. **DONE (#280):** moved verbatim to `worker/legacy/cloud/keeper.py`; its checks are NOT yet re-expressed as fixtures (only `gcloud.py`’s were, per #284) — see `worker/legacy/cloud/README.md`. |
| `worker/src/dna_entropy/cloud/orchestrator.py` | Zone/offer escalation (`_create_box`, `_try_sequential`/`_try_parallel`), VM setup script, `run_in_cloud` (find/wake/upload/run/download, never creates/deletes), constants (`BOX_NAME`, `OFFERS`, `DEFAULT_ZONES`, `BASE_IMAGE_FAMILY`). | GUT | Zone-ladder + escalation logic is valuable but must be reimplemented in C# against the Compute API directly (§5.4); the box-reuse-by-fixed-name model is replaced by per-job VMs found by label. | GUT from the shipped package; **REFERENCE** for the zone ladder, GPU offer table, DLVM image constants, and VM setup script shape — see "Prototype behaviours" below. **DONE (#280):** moved verbatim to `worker/legacy/cloud/orchestrator.py`; the zone ladder/offer table/quota metrics are already transcribed in `docs/superpowers/specs/2026-09-18-appendix-b-cloud-design.md` §4.4 and its model/hardware matrix table (predates this gut, already satisfies #280’s design-doc requirement). |
| `worker/src/dna_entropy/cloud/ui.py` | ASCII `Spinner` context manager for CLI progress. | GUT | Console-only UX; the WinUI app has its own run-progress screen (§4.4) and the worker reports progress via `status.json`/`progress.jsonl`, not stdout spinners. | Not needed; no reference value beyond illustrating "never crash on a UI helper" pattern. **DONE (#280):** moved verbatim to `worker/legacy/cloud/ui.py` anyway (kept with its siblings rather than deleted, since #280 asks for the whole package untouched as reference). |
...
| `worker/src/dna_entropy/predictors/evo.py` | `EvoPredictor`: Evo 2 wrapper, tokenizer-derived nucleotide ids, robust logits extraction, bf16, only module allowed to import `torch`/`evo2`. | ADAPT | Needs a `max_context` that comes from the manifest's window size `W` (not a fixed 8192), and a `MODEL_NEEDS_HOPPER` gate for `evo2_40b`/`evo2_1b_base` (design D11/D16, §5.5). | Stays in place; extended, not replaced. **DONE (#279, #282):** `max_context` now comes from `cfg.max_len` via `pipeline.build_predictor` (was silently defaulting to 8192, a real bug caught while testing #279); `predictors/hardware.py`'s `require_hardware` gate is called before `Evo2(model)` loads any weights, refusing `evo2_1b_base`/`evo2_20b`/`evo2_40b` on any device whose reported compute capability is below Hopper (sm_90), including the no-CUDA case. |
...
| `worker/src/dna_entropy/readers/input.py` | `load_input()`/`LoadedInput`/`Contig`: unified routing + output-safe contig naming. | ADAPT | Needs to route every FASTA record into its own `Contig` once `readers/fasta.py` returns all records (companion change to D14). | Stays; minor extension. **DONE (#283):** the FASTA branch now loops over every record exactly like the GenBank branch already did, using the same `_safe_contig_name(cfg.name, i, total)` scheme (collision-proof regardless of FASTA header text) and carrying each record's header forward as `Contig.source_id`. |
...
| `worker/keep_gpu.py` | Standalone entry point for the always-on keeper (`python keep_gpu.py`). | GUT | Thin wrapper around `cloud.keeper.main`, which is gutted (D1). | Not shipped. **DONE (#277):** deleted; its only logic (two lines calling `cloud.keeper.main`) has no reference value beyond what `worker/legacy/cloud/keeper.py` already preserves verbatim. |
...
| `worker/keep-gpu.spec` | PyInstaller spec for the keeper `.exe`. | GUT | Same reasoning as `keep_gpu.py`/`cloud/keeper.py`. | Not shipped. **DONE (#277):** deleted alongside `keep_gpu.py`. |
...
**Tracked by** #57 (`CloudErrorClassifier` from structured operation errors, which is where
`classify_create_error` lands) and #213 (the prototype's classification and quota-parsing cases
re-expressed as fixtures for the C# port). The extracted vectors themselves are already in the repo at
`tests/contract-fixtures/cloud_error_classification.json` and `cloud_quota_parsing.json`, written by #284.
...
**Tracked by** #86 (the zone and offer escalation ladder) and #210 (last-good-zone and per-region quota
memory persisted per project). The Compute operation polling and backoff half lands in #256.
...
**Tracked by** #204 (`SetupHealthService` ports `_diagnose_setup`'s four rows with remediation ids),
#86 (`_apply_quota_health`'s unknown-versus-zero-versus-narrowed quota pre-filter), #74 (the GPU health
probe, which moves into the startup script rather than the app), #59 and #90 (the watchdog and its
deadlines, re-expressed as `JobReconciler`), #93 (the keep-alive idle timer, which is new design
behaviour rather than a port), and #113 and #83 (the nothing-left-running scan, in the app and in CI).

`keep_alive`'s "never stop or delete" invariant is deliberately **inverted** by D10: the app does stop and
delete VMs. What carries over is the discipline around it, not the rule.
