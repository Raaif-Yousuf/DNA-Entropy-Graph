...
| `analysis.contextLength` / `window` / `stride` | `K`, `W`, `S` from `science_and_formats.md` section 3 | `window` and `stride` are **derived, not chosen** by the user, but recorded here so the worker does not have to re-derive them and so `provenance.json` and the manifest agree by construction. **RE-DECIDED 2026-09-19 (issue #345), correcting this row's own earlier "settled, no gap" claim**: `window` **is** consumed (`build_run_config` reads `analysis.window` as `RunConfig.max_len`, the worker's ceiling); `stride` is **not** consumed anywhere in `build_run_config`. The earlier claim reasoned correctly that the WORKER's own `S = W - K` can never independently drift, but missed that the manifest's declared `stride` is itself a third, independently-writable value — a manifest declaring a `stride` that disagreed with `window - contextLength` was silently ignored rather than refused, which is a real gap, not a documented non-issue. Fixed: `AnalysisSpec.from_dict` (`worker/manifest.py`) now cross-checks `stride == window - contextLength` at parse time and raises `ManifestError` on a mismatch, the same way `predictor.precision` (issue #343) is cross-checked below. |
| `predictor.precision` | `bf16 \| fp8` | **Fixed 2026-09-19 (issue #343)**: accepted any string with no relationship to the model id, even though nothing downstream ever reads this field to configure the predictor - `predictors/hardware.py`'s `MODEL_REQUIREMENTS` table alone determines the real precision used, gated on the device's actual reported compute capability, never on a manifest-declared string. A manifest could previously declare `"bf16"` for a Hopper-only model (or vice versa) with nothing to catch the disagreement. Now cross-checked at parse time against `predictors.hardware.model_requirement(model).precision` and rejected with a named error on a mismatch - but only for the real (`"evo"`) predictor (irrelevant for `"mock"`) and only for a model id `predictors/hardware.py` actually recognizes (an unrecognized id is `predictors.hardware`'s own `UnknownModelError`/`MODEL_UNKNOWN` refusal, issue #346, raised later with a clearer, dedicated message that this check must not preempt). |
