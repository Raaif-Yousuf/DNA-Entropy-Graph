...
**`docs/contract/error-codes.json` is the machine-readable source for *which codes
exist*** — generated by `scripts/gen_manifest_schema.py --check` directly from
`dna_entropy.worker.errors.WORKER_ERROR_CODES`, the worker's own registry, so it cannot
silently drift from what the Python code actually raises the way a hand-maintained list
copied from the spec eventually would. This document remains the authority for *what each
one says to a user*. The two are meant to agree, checked by
`test_every_worker_code_is_in_copy_catalog_or_has_a_documented_gap_note` in
`worker/tests/test_worker_errors.py`: every code in the generated file must either appear
as a row below, or carry a non-empty `note` in the Python registry explaining exactly why
it does not. As of this revision every one of the nine worker codes has a row below -
`MANIFEST_INVALID` and `WORKER_VERSION_MISMATCH` were the two carrying a documented-gap
note instead (an app-only failure "a user should never see," per the registry's own
reasoning); both are added below since a user can in fact hit `WORKER_VERSION_MISMATCH`
in practice (an app update and the pinned worker image landing out of step), and giving
`MANIFEST_INVALID` a plain-language fallback costs nothing even if it should be rare. The
Python registry's own `note` text for both still describes the old, now-superseded
disposition; that is a small staleness in `worker/errors.py`, outside this document's own
path, flagged in this session's report rather than edited directly. The next person
reconciling this table against reality should start from the generated file, not from the
spec's older list, since the spec predates several codes the worker actually raises today.
...
| `WORKER_VERSION_MISMATCH` | `WorkerVersionMismatch` | Your app and your analysis software don't match | This app expects a different version of the analysis software than the one that actually ran. This usually clears up on its own once an in-progress app update finishes installing. | [Update the app] |
| `MANIFEST_INVALID` | `ManifestInvalid` | Something went wrong before your analysis could start | The information this app sent to start your run was invalid. This is a problem with the app, not with your sequence file. | [Report a problem] |
